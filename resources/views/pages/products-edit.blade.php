...
<div class="row">
    <div class="col-12 mb-1">
        ...
        <div class="float-right">
            <a class="btn btn-primary" href="{{asset('products/list')}}"> Back</a>
        </div>
    </div>
</div>
@if (count($errors) > 0)
...
<section class="users-edit">
  <div class="card">
    <div class="card-body">
            ...
            <form class="form-validate" method="POST" action="{{route('products-update',$product->id)}}">
              {{ csrf_field() }}
                <div class="row">
                  ...
                  <div class="col-12 col-sm-6">
                      <div class="form-group">
                        <div class="controls">
                            <label>System</label>
                        <select class="form-control select2" name="system_id">
                          <option value="">Select System</option>
                          @foreach ($systems as $key => $system)
                            <option value="{{$system->id}}" {{ ($system->id == $product->system_id) ? ' selected' : '' }}>{{$system->name}}</option>
                          @endforeach
                        </select>
                        </div>
                      </div>
